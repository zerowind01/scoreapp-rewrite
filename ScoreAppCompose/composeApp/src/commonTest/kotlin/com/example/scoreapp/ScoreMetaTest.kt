...
class ScoreMetaTest {
    ...
    @Test
    fun `UTC 偏移为 0 时按时戳原样格式化`() {
        // 1970-01-02 00:00 UTC（1 天），验证日期进位与补零
        assertEquals("1970-01-02 00:00", formatDate(86_400_000L, 0))
    }

    @Test
    fun `东八区下添加时间不再早 8 小时`() {
        // 同一个时刻：UTC 是 00:00，本地（UTC+8）应是 08:00。
        // 原实现直接按 UTC 解释时间戳，在中国会显示成 00:00 —— 整整早 8 小时。
        assertEquals("1970-01-02 08:00", formatDate(86_400_000L, 8 * 3600))
    }

    @Test
    fun `负偏移回退到前一天`() {
        // epoch 当天的 00:00，在 UTC-5 下仍属 1969-12-31 19:00
        assertEquals("1969-12-31 19:00", formatDate(0L + 1L, -5 * 3600))
    }

    @Test
    fun `跨日的偏移会推动日期`() {
        // 1970-01-01 20:00 UTC，东八区已是 1970-01-02 04:00
        assertEquals("1970-01-02 04:00", formatDate(20 * 3600 * 1000L, 8 * 3600))
    }
    ...
    fun `月日时分补零`() {
        ...
        assertEquals("2021-01-02 03:04", formatDate(1_609_556_640_000L, 0))
    }
