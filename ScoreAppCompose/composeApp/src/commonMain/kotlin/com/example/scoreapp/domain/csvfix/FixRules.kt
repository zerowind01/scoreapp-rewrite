...
object FixRules {
    ...
    /**
     * 把一串原始标题按 [TITLE_RULES] 全部跑一遍，得到界面上显示的那个干净标题。
     *
     * 单独抽出来是因为它有两个调用方：**规则提议**（决定要不要改这一行的标题）
     * 和 **AI 相关性守卫**（[AiFill.looksRelated] 比对不对得上）。
     * 两边必须走同一份清洗 —— 否则会出现「屏幕上明明显示「月光」，
     * 守卫却拿「月光pdf」去比」这种自相矛盾的行为。
     */
    fun cleanTitle(raw: String): String {
        var cur = raw
        TITLE_RULES.forEach { rule -> cur = rule.run(cur) }
        return cur
    }
